...
- Private send-prep page → `x/sendprep-500f2904f3/index.html` (JK only: the four standalones under submission, their cover letters, and the open cold-read/open-calls cards, with Take/Leave marks, paragraph notes and Gate 0; opens on a home page of six tiles, each section its own address `#bugis` `#friend` `#interval` `#first` `#other` `#send`; **sealed**: AES-256 in the page, opened in the browser with a passphrase that lives only in CASA FORTUNA `_submissions/_web/.sendprep_web_secret.json`; Netlify-Forms inbox `sendprep-notes`). Generated by CASA FORTUNA `_submissions/_web/build_sendprep_web.py` (close_casa.sh runs it); never hand-edit, never delete as an orphan.
